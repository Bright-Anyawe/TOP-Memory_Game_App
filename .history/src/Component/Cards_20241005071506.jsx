...
function Card() {
 ...
 const request7 = fetch(
      ...
      try {
        const response = await Promise.all([request1, request2, request3, request4, request5, request6, request7, request8]);
        console.log(response);
        const responseJson = response.map((data) => data.json());

          const awaitJson = await Promise.all(responseJson);
          console.log(awaitJson)
          const imageUrl = awaitJson.map(
        ...
        setCards((prevCards) =>
          prevCards.map((card, index) =>({ ...card, url: imageUrl[index] || card})
          )
        );
        console.log(imageUrl);
      } catch (error) {
  ...
  function handleClick() {
    console.log(`This ${cards.map(card => return card.name)} has been click`)
  }
  ...
  return (
    <>
      ...
      <ul>
        {cards.map((card) => (
          <li key={card.name} className="card" onClick={handleClick}>
